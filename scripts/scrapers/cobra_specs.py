...
COBRA_DRIVERS_URL = "https://www.cobragolf.com/collections/drivers"


def parse_cobra_card(html: str) -> dict:
    """Parse a single Cobra product card HTML into a spec dict."""
...
async def scrape_cobra_drivers() -> list[dict]:
    """Scrape current Cobra drivers from cobragolf.com."""
    clubs = []
    async with get_browser() as browser:
        page = await new_page(browser)
        logger.info("Navigating to Cobra drivers page")
        try:
            await page.goto(COBRA_DRIVERS_URL, wait_until="networkidle", timeout=30000)
            await delay()

            cards = await page.query_selector_all(
                "[class*='product-card'], [class*='ProductCard'], .product-tile, [class*='collection-product']"
            )
            ...
            for card in cards:
                ...
                try:
                    club = parse_cobra_card(html)
                    if club["model_name"]:
                        clubs.append(club)
                except Exception as e:
                    logger.warning(f"Failed to parse Cobra card: {e}")
        except Exception as e:
            logger.error(f"Cobra scrape failed: {e}")
            raise
        finally:
            await page.close()

    logger.info(f"Scraped {len(clubs)} Cobra drivers")
    return clubs
