...
CALLAWAY_DRIVERS_URL = "https://www.callawaygolf.com/golf-clubs/drivers/"


def parse_callaway_card(html: str) -> dict:
...
async def scrape_callaway_drivers() -> list[dict]:
    """Scrape current Callaway drivers from callawaygolf.com."""
    clubs = []
    async with get_browser() as browser:
        page = await new_page(browser)
        logger.info("Navigating to Callaway drivers page")
        try:
            await page.goto(CALLAWAY_DRIVERS_URL, wait_until="networkidle", timeout=30000)
            await delay()
            ...
            for card in cards:
                ...
                try:
                    club = parse_callaway_card(html)
                    if club["model_name"]:
                        clubs.append(club)
                except Exception as e:
                    logger.warning(f"Failed to parse card: {e}")
        except Exception as e:
            logger.error(f"Callaway scrape failed: {e}")
            raise
        finally:
            await page.close()

    logger.info(f"Scraped {len(clubs)} Callaway drivers")
    return clubs
