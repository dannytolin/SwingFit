...
TAYLORMADE_DRIVERS_URL = "https://www.taylormadegolf.com/drivers/"


def parse_taylormade_card(html: str) -> dict:
...
async def scrape_taylormade_drivers() -> list[dict]:
    """Scrape current TaylorMade drivers from taylormadegolf.com."""
    clubs = []
    async with get_browser() as browser:
        page = await new_page(browser)
        logger.info("Navigating to TaylorMade drivers page")
        try:
            await page.goto(TAYLORMADE_DRIVERS_URL, wait_until="networkidle", timeout=30000)
            await delay()
            ...
            for card in cards:
                ...
                try:
                    club = parse_taylormade_card(html)
                    if club["model_name"]:
                        clubs.append(club)
                except Exception as e:
                    logger.warning(f"Failed to parse card: {e}")
        except Exception as e:
            logger.error(f"TaylorMade scrape failed: {e}")
            raise
        finally:
            await page.close()

    logger.info(f"Scraped {len(clubs)} TaylorMade drivers")
    return clubs
